...
If one resource needs to be loaded, that is enqueued in a special queue. What is so special about this queue is that it is prepared for being accessed by several threads (thread safe queue). This type of queue is usually called work queue.
...
Physics simulation is accurate thanks to the, moving the origin of coordinates, strategy. We have wrapped a physics engine but all the concepts used are applicable to any other physics engine.
